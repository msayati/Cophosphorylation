inFolder = 'out/figures/paper_output/figure1/';
load([inFolder, 'figure1a_stats_data_all.mat']);
%%
M = Info.MeanDif';
T = Info.TStat;
CI = Info.MeanDifCI';
...
for iRow = 1:nRow
    for iColumn = 1:nColumn
        mVal = M(iRow, iColumn);
        mText = sprintf('\\fontsize{%d} %.2f', 12, mVal);
        ciText = sprintf('\\fontsize{%d} [%.2f, %.2f]', 11, CI(1, iColumn), CI(2, iColumn));
...
set(gcf, 'Color', [1 1 1]);
set(gcf, 'Position', [0 0 1100 80]);
movegui('center');
export_fig([inFolder, 'figure1a_stats_bar.png'], '-m2');
