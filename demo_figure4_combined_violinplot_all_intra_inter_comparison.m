...
Infog = table();
Infog.Datasets = dataset_labels';
Infog.nSiteTotal = zeros(nDataset, 1);
Infog.nProteinTotal = zeros(nDataset, 1);
Infog.nSiteNZ = zeros(nDataset, 1);
Infog.nEdge = zeros(nDataset, 1);
...
%                     Xp = X(randperm(nSite), :);
        ...
        W_base_x = W_base;
        W_base_x = W_base_x - diag(diag(W_base_x));
        W_base_x = W_base_x | W_base_x';
        
        Infog.nSiteTotal(iDataset) = length(W_base);
        Infog.nProteinTotal(iDataset) = length(unique(Sites_base.Protein));
        Infog.nSiteNZ(iDataset) = nnz(sum(W_base_x)>0);
        Infog.nEdge(iDataset) = nnz(W_base_x - diag(diag(W_base_x)))/2;
    end
end
% D = horzcat(D_all{:});
% Dp = horzcat(Dp_all{:});
...
for iSide = 1:2
    Infox = table();
    Infox.Dataset = dataset_labels';
    Infox.MeanDif = nan(nDataset, 1);
    Infox.MeanDifCI = nan(nDataset, 2);
    Infox.TStat = nan(nDataset, 1);
    Infox.PValues = nan(nDataset, 1);
    Infox.KSpval = nan(nDataset, 1);
    for iDataset = 1:nDataset
        % Left - Right
       Dnetwork = D_all{iSide, iDataset};
       Dall = Dp_all{iSide, iDataset};
       [h, p, ci, stats] = ttest2(Dnetwork, Dall);
       mdif = mean(Dnetwork) - mean(Dall);
       Infox.SampleNcolored(iDataset) = length(Dnetwork);
       Infox.SampleNgrayed(iDataset) = length(Dall);
       Infox.MeanDif(iDataset) = mdif;
       Infox.MeanDifCI(iDataset, :) = ci;
       Infox.TStat(iDataset) = stats.tstat;
       Infox.PValues(iDataset) = p;
       if(~isempty(Dnetwork))
           [h, p, ksstat] = kstest2(Dnetwork, Dall);
           Infox.KSpval(iDataset) = p;
       end
    end
    ...
    switch(iSide)
        ...
        case 2
            InfoLR.Right = Infox;
        otherwise
            error('Invalid plot side');
    end
end
figure_path = sprintf('out/figures/paper_output/figure4/');
if(~exist(figure_path, 'dir')); mkdir(figure_path); end
save([figure_path, 'figure4_', lower(networkname), '_stats_data.mat'], 'InfoLR');
...
figure_path = sprintf('out/figures/paper_output/figure4/');
if(~exist(figure_path, 'dir')); mkdir(figure_path); end
figure_name = sprintf('%s_violinplot_intra_inter_%s_%s', ...
