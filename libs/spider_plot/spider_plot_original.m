function spider_plot_original(P, varargin)
...
axes_scaling = 'linear';
log10labeling = false(num_data_points, 1);
nAxes = size(P, 2);
...
if numvarargs > 1
    ...
    for ii = 1:length(name_arguments)
        ...
        switch lower(name_arguments{ii})
            ...
            case 'log10labeling'
                log10labeling = value_arguments{ii};
            otherwise
...
%         P(:, iAxes) = sign(P(:, iAxes)) .* log10(abs(P(:, iAxes)));
%     end
% end
...
%         text_str = sprintf(sprintf('%%.%if', axes_precision), axes_value);
        if(jj >= 2 && log10labeling(ii))
            axes_log10_val = axes_value;
            if(floor(axes_log10_val)==axes_log10_val) % If integer
                text_str = sprintf('10^%d', axes_log10_val);
            else
                val = 10^axes_log10_val;
                tens = floor(axes_log10_val);
                remain = round(val / 10^tens);
                if(remain == 1)
                    text_str = sprintf('%10^%d', tens);
                else
                    text_str = sprintf('%dx10^%d', remain, tens);
                end
                text_str = sprintf(['10^{%.', num2str(1), 'f}'], axes_log10_val);
            end
        else
            text_str = num2str(axes_value, axes_precision);
%             text_str = sprintf(sprintf('%%.%if', axes_precision), axes_value);
            if(axes_value == 0)
                text_str = '0';
            end
