...
if numvarargs > 1
    ...
    for ii = 1:length(name_arguments)
        ...
        switch lower(name_arguments{ii})
            ...
            case 'horzcenterall'
                horzcenterall = value_arguments{ii};
            otherwise
...
for ii = 1:num_data_points
    ...
    if isempty(axes_limits)
        ...
        P_scaled(:, ii) = ((group_points - min_value) / range) * (1 - rho_increment);
    else
...
%%% Polar Axes %%%
% Polar coordinates
rho = (0:rho_increment:1) - rho_increment;
...
for ii = 1:theta_end_index
    ...
    for jj = 2:length(rho)
        ...
        % Display axes text
        if(jj >= 3 && log10labeling(ii))
            if(isinteger(axes_value))
                text_str = sprintf('10^%d', axes_value);
            else
                text_str = sprintf(sprintf('10^%%.%if', axes_precision), axes_value);
            end
        else
