...
for iDataset = 1:nDataset
    ...
    switch(network)
        ...
        case 'random'
%             W = true(nSite, nSite);
            indices = randi([1 nSite], 2e7, 2);
            indices = indices(indices(:, 1) < indices(:, 2), :);
            W = sparse(indices(:, 1), indices(:, 2), 1, nSite, nSite);
            W = W | W';
            networkname = 'AllPairsRandom';
        case 'intraprotein'
            W = Wintra_protein;
...
for iDataset = 1:nDataset
    % Left - Right
   Dleft = D_all{1, iDataset};
   Dright = Dp_all{1, iDataset};
...
figure_path = sprintf('out/figures/paper_output/figure1/');
if(~exist(figure_path, 'dir')); mkdir(figure_path); end
save([figure_path, 'figure1a_stats_data.mat'], 'Info');
...
%             'MeanLineColor', 'groupcolor');
        h2 = violinplot(Dx_all(:, iDataset), 'XShift', iDataset-1, ...
...
figure_path = sprintf('out/figures/paper_output/figure1/');
figure_name = sprintf('violinplot_all_permuted_%s_%s', fctext, measure);
if(~exist(figure_path, 'dir')); mkdir(figure_path); end
